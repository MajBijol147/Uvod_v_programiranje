...
def pravilna_koda(niz):
    for i, znak in enumerate(niz):
        if znak == "=":
            ...
            if "=" in niz[:i]:
                pass
            elif st == 3 and niz[i - 1] not in kodirna_tabela["000000"]:
                return False
            elif st == 2 and niz[i - 1] not in [
                kodirna_tabela["010000"],
                kodirna_tabela["100000"],
                kodirna_tabela["110000"],
            ]:
                return False
            elif st == 1 and niz[i - 1] not in [
                kodirna_tabela["000100"],
                kodirna_tabela["010100"],
                kodirna_tabela["100100"],
                kodirna_tabela["110100"],
                kodirna_tabela["110100"],
                kodirna_tabela["001000"],
                kodirna_tabela["011000"],
                kodirna_tabela["101000"],
                kodirna_tabela["111000"],
                kodirna_tabela["001100"],
                kodirna_tabela["011100"],
                kodirna_tabela["101100"],
                kodirna_tabela["111100"],
            ]:
                return False
        elif znak not in kodirna_tabela.values():
            return False
    return True
...
def zakodiraj(niz):
    ...
    while len(niz) % 6 != 0:
        ...
        dodano = dodano + "="
    st_kosov = len(niz) // 6
    for k in range(1, st_kosov + 1):
        sez.append(niz[(k - 1) * 6 : k * 6])
    for kos in sez:
        zakodirano = zakodirano + kodirna_tabela[kos]
    return zakodirano + dodano
...
# =============================================================================
obrnjena_kodirna_tabela = {v: k for k, v in kodirna_tabela.items()}
...
def odkodiraj(niz):
    ...
    for znak in niz:
        if znak == "=":
            ...
            binarno = binarno + obrnjena_kodirna_tabela[znak]
    return binarno[: len(binarno) - 2 * dodano]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIxMSwidXNlciI6MTA3MzB9:1uOxov:qcKKF_21awfmuTbjv0pb572VzCMV20Q9wOkeBCvRcYk"
        try:
            kodirna_tabela = {
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIxMiwidXNlciI6MTA3MzB9:1uOxov:e3re2c6irhfzeWQIb43nYGeAQkcy8Qezu9UTx8uWnEM"
        try:
            testi = [
                ('000000', 'A'),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDIxMywidXNlciI6MTA3MzB9:1uOxov:6fhaLVH31w-YTE8cQs0uJSVThHNq_a5Vw89HWbZCURw"
        try:
            testi = [
                ('000000', 'A'),
                ('011000100110000101111010011000010011011000110100', 'YmF6YTY0'),
