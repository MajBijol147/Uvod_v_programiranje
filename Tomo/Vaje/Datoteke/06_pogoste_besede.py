    ...
    with open(vhod, encoding="utf-8") as vh:
        for vrstica in vh:
            if vrstica == "":
                ...
                for locilo in ".,;:-_!?":
                    ociscena_vrstica = vrstica.strip(locilo)
                besedilo = besedilo + ociscena_vrstica
    razdeljeno_besedilo = besedilo.split(" ")
    pojavitve = {}
    for beseda in razdeljeno_besedilo:
        pojavitve.update({beseda: razdeljeno_besedilo.count(beseda)})
    return pojavitve[: st_besed + 1]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODkxLCJ1c2VyIjoxMDczMH0:1uOHiM:2DsSBA-6pdsqmSBatl-DA6oO5lpr88_Lbty5Vg8sUVw"
        try:
            pass
        except TimeoutError:
