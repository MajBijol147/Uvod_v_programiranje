...
def stirling_vsota(n, k, j=0):
    return (sum(((-1)**j) * binomski_fakulteta(k, j) * (k - j)**n for j in range(k + 1)))
# =====================================================================@020027=
# 3. podnaloga
...
def stirling_direktno(n, k):
    return stirling_vsota(n, k, j=0) // fakulteta(k)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDAyMywidXNlciI6MTA3MzB9:1tppSL:mEJqRAvTbjA_9RBxwMGDG_bsIis_HQri9HIQ4n9oyls"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDAyNCwidXNlciI6MTA3MzB9:1tppSL:2X_k6EzRMy0-moair68KzF2AW3Eirz5FV6IgbOFBpWQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDAyNywidXNlciI6MTA3MzB9:1tppSL:GUA1hdKNcBOt40LE7MGCb97ykhidx9vdVE718tHKpUw"
        try:
