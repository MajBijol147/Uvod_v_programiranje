...
def obrat(n):
    ...
    nove_enice = enice * 100
    nove_stotice = stotice
    
    return nove_enice + desetice * 10 + nove_stotice
# =====================================================================@000926=
# 3. podnaloga
...
def dodaj_kontrolno_stevko(sklic):
    ...
    produkti_stevk = (s1 * 2) + (s2 * 3) + (s3 * 4) + (s4 * 5) + (s5 * 6) + (s6 * 7) + (s7 * 8) + (s8 * 9) + (s9 * 10) + (s10 * 11) + (s11 * 12) + (s12 * 13)
    
    kontrolna_stevka = 11 - (produkti_stevk % 11)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjEwNzMwfQ:1tmvOv:nMf17Q2A-TFCiZrfecaG4tceO5tjAs5BWWfXhbmc5qs"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjEwNzMwfQ:1tmvOv:qcdFZDqz3B-lEdK5AV8QdurOHnNGr-Erbm3770ZLNak"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjEwNzMwfQ:1tmvOv:hbF9uw0UV53M23KQcJ9L3dvTpkCnNbwyRy6AwzgVDY0"
        try:
