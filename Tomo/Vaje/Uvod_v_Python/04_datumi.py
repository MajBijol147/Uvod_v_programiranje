...
def stevilo_dni(mesec, leto):
    dolgi_meseci = [1, 3, 5, 7, 8, 10, 12]
    if mesec in dolgi_meseci:
        return 31
    else:
        if mesec == 2 and je_prestopno(leto) == True:
                return 29
...
def je_veljaven_datum(dan, mesec, leto):
    if mesec in range(1, 13) and dan in range(1, 32):  
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjcsInVzZXIiOjEwNzMwfQ:1tmw00:Kbyi4QaxhhWDtlF2JKLpleDC350_64cYy53BP3q0oaI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjgsInVzZXIiOjEwNzMwfQ:1tmw00:jc4LdNzZ7IS4e3_ng-rZ3T1dDdn0s_rwiJLUfuUdO_4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjksInVzZXIiOjEwNzMwfQ:1tmw00:RV9cNrlpDEFA0TxLJij170m-c0cSAzFPST4APtSZJRE"
        try:
